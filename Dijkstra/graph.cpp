...
Graph::Graph(int n, int first, int last, string vertexnames[]) {
	numOfVerts = n;
	start = first;
	finish = last;
	...
	printAdjMat();
	if ((start != finish) & (start >=0 & finish >= 0) & (start < numOfVerts & finish < numOfVerts)){
		dijkstra();
		printPath(finish);
	}
	else if (start == finish) {
		cout << "selected start index = selected finish index...  There is no path" << endl;
	}
	else if (start < 0 | finish < 0) {
		cout << "start or finish were set less than 0" << endl;
	}
	else {
		cout << "start or finish are out of range" << endl;
	}
}
...
void Graph:: dijkstra(){
	...
	for (int i = 0; i < numOfVerts; i++) {
		if ((adjMatrix[start][i] > 0) & (adjMatrix[start][i] + distances[start] < distances[i])){
			distances[i] = adjMatrix[start][i] + distances[start];
			prev[i] = start;
		}
	}
	//Step 3:
	...
	while (1) {
		int nextIdx = minDistance();
		if (nextIdx == -1) {
			break;
		}
		setDistances(nextIdx);
	}


}

void Graph::setDistances(int latestVert) {
	//8 pts
	...
	for (int i = 0; i < numOfVerts; i++) {
		if ((adjMatrix[latestVert][i] > 0) & (adjMatrix[latestVert][i] + distances[latestVert] < distances[i])) {
			distances[i] = adjMatrix[latestVert][i] + distances[latestVert];
			prev[i] = latestVert;
		}
	}
