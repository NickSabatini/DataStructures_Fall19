...
public: 				/* Yeah, everything’s public. No getters or setters. Deal with it. */
	 ...
	 * values of the two children (it calls findMax for that). If the value at the index is less than the value of
	 * the greatestchild, it swaps with the greatest child and the index becomes that of the greatest of the two
	 * children. It continues until either it can’t bubble down any further or until neither child is greater
	 * than the parent (the value at the index
	 */

	void bubbleUp(int i); // 7 pts
	/* this method you’re writing – it takes an index (the last index in the HEAP (not the array) and compares
	 * the value in the index with the value of its parent. If it is greater than the value of the parent,
	 * they swap and the current index becomes that of its parent. It continues until the two don’t swap or
